...
<div class="border border-gray-200 rounded overflow-x-auto min-w-full bg-white">
  ...
  <table class="min-w-full text-sm align-middle whitespace-nowrap">
    ...
    <!-- Table Body -->
    <tbody v-for="i in this.$store.state.sunucularDataTable" :key="i._id">
      <tr>
        <td class="p-3 text-center">
          <img :src="`${i.images}`" alt="User Avatar" class="inline-block w-10 h-10 rounded-full" />
        </td>
        <td class="p-3">
          <p class="font-medium">
            {{i.title}}
...
export default {
  ...
  mounted(){
    this.$store.dispatch("getSunucularDataTable")
  },

  methods:{
    onayla(i){
      axios.get('http://localhost:5000/onayla', {headers: {id: i}})
          .then(res => {
             this.$swal({
                icon: 'info',
